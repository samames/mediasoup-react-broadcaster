...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ room_id, name }, cb) => {
    ...
    roomList.get(room_id).sendCount(roomList.get(room_id).getPeers().size);
  ...
  socket.on('disconnect', () => {
    ...
    if (!roomList.get(socket.room_id)) return;
    ...
    if (roomList.get(socket.room_id).getPeers().size === 0) {
      ...
      roomList
        .get(socket.room_id)
        .sendCount(roomList.get(socket.room_id).getPeers().size);
    }
  });
